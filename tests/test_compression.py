...
def test_compression(inner_size):
    ...
    high = (
        2 ** (n_bits - 1) if signed else 2**n_bits
    ...
    inner_size_rounded_input_poly = (
        round_up(a.shape[1], params_dict["polynomial_size"]) * a.shape[0]
    )
    inner_size_rounded_output_poly = a.shape[0] * round_up(
        b.shape[1], params_dict["packing_ks_polynomial_size"]
    )
    ...
    print(
        f"Input expansion factor: {len(serialized_ciphertext) / inner_size_rounded_input_poly:.2f}"
    )
    ...
    print(
        f"Output expansion factor: {len(serialized_encrypted_result) / inner_size_rounded_output_poly:.2f}"
    )
