...
def is_latest_entry(args):
    ...
    if VersionInfo.is_valid(new_version_str):
...
def _check_version(pyproject_file, file_vars, version):
    ...
    new_version = next(iter(version_str_set))
    if not VersionInfo.is_valid(new_version):
        raise RuntimeError(f"Unable to validate version: {new_version}")

    print(f"Found version {new_version} in all processed locations.")
